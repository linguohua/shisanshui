...
func GenToken(id string) string {
	var plainTK = fmt.Sprintf("%s@%d", id, timeNow())
	log.Println("GenTK, plainTK is:", plainTK)
	return encrypt(mykey, plainTK)
}

// ParseToken verify and extact id from a token
func ParseToken(token string) (string, bool) {
	...
	//log.Println("ParseToken, plain token is:", plainTK)
	...
	var now = timeNow()
	//log.Printf("ParseTK, id:%s, timestamp:%d, now:%d", splits[0], timestamp, now)
